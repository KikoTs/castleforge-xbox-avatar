...
#if XBOX_AVATAR_BRAND || CASTLEFORGE_BRAND
...
#endif
{
    ...
    internal static class ItemTuning
    {
        ...
        /// <summary>
        /// How far the first-person hand follows the game's own finger pose:
        /// 0 is the open hand third person shows, 1 the pose the first-person
        /// clip holds - the fist around a pickaxe, the finger on a trigger.
        /// The glove holds at either end; it is in the tuning file for taste.
        /// </summary>
        private const float DefaultGrip = 1f;
        ...
        private static void WriteTemplate(string path)
        {
            ...
            File.WriteAllLines(path, new[]
            {
                ...
                "hands mesh",
                "",
                "# How far the 'mesh' hand follows the game's own finger pose.",
                "# 1 is exactly what the game's first-person animation does -",
                "# the fist round a pickaxe, the finger on a trigger - and 0 is",
                "# the open hand third person shows; anything between is fine.",
                "# Only the fingers move, the glove stays whole at every setting.",
                "#",
                "#     grip 1",
                "",
                "grip 1",
    ...
    internal static class Branding
    {
        ...
        // CastleForge gives every mod a folder of its own under !Mods, named
        // after the mod, and extracts native dependencies into a Natives
        // subfolder beneath it. Keeping to that layout puts the avatar, its
        // cache and the capture bridge where a CastleForge user already knows
        // to look, and makes uninstalling the deletion of one folder.
        internal const string ProductName = "Xbox Avatar";
        private static readonly string[] AvatarSegments = { "!Mods", "XboxAvatar" };
        private static readonly string[] BridgeSegments = { "!Mods", "XboxAvatar", "Natives" };
    ...
    internal sealed class ImportedAvatarModelEntity : StockModelEntity
    {
        ...
        /// Xbox bone frames are the opposite handedness from XNA's, which
        /// third person handles by conjugating every local transform with a Z
        /// reflection; going straight from the avatar's bind-local frame onto
        /// the XNA proxy bone needs that same reflection, once per pinned
        /// bone. Without it every bone's geometry was mirrored across its own
        /// hinge plane: a sleeve is symmetric enough to hide that, a hand is
        /// not. The hand came out thumb-for-pinky, and because each finger
        /// bone was mirrored in its own frame while the wrist was mirrored in
        /// its, the glove tore open at the knuckles - 107 triangles stretched
        /// past 1.8x, the worst 19x, on both sides of the palm at once.
        ///
        /// Below each wrist the hand is built the way third person builds
        /// every animated bone: the proxy's own joint rotation, relative to
        /// its parent and conjugated into the avatar's handedness, on the
        /// avatar's own bone offsets. That puts every fingertip within a
        /// millimetre of where the game's first-person clip puts it - the
        /// pickaxe fist, the pistol's trigger finger, the support hand - with
        /// the glove whole, because the rotations are now applied about the
        /// avatar's own joints. <paramref name="grip"/> blends each joint
        /// from the avatar's open bind pose (0) to the game's pose (1).
        ///
            ...
            Matrix[] live)
        {
            ...
            Matrix reflectZ = Matrix.CreateScale(1f, 1f, -1f);
            for (int bone = 0; bone < boneCount; bone++)
            {
                ...
                {
                    Vector3 shape = asset.FirstPersonBoneScale[bone];
                    live[bone] =
                        Matrix.CreateScale(shape.X, shape.Y, -shape.Z) *
                ...
                else
                {
                    Matrix proxyLocal =
                        proxyWorld[proxyBoneByAvatar[bone]] *
                        Matrix.Invert(proxyWorld[proxyBoneByAvatar[parent]]);
                    live[bone] =
                        RetargetHandLocal(
                            reflectZ * proxyLocal * reflectZ,
                            asset.SourcePoseLocal[bone],
                            grip) *
                        live[parent];
                }
        ...
        /// <summary>
        /// A hand bone's local transform: the avatar's own offset and scale,
        /// with its rotation blended from the avatar's bind pose towards the
        /// animated joint rotation. The same retargeting third person applies
        /// to every bone, with a blend so the grip can be opened.
        /// </summary>
        private static Matrix RetargetHandLocal(
            Matrix animated,
            Matrix sourcePose,
            float grip)
        {
            ...
            {
                animatedRotation = sourceRotation;
            }
            Quaternion rotation = grip >= 1f
                ? animatedRotation
                : grip <= 0f
                    ? sourceRotation
                    : Quaternion.Slerp(sourceRotation, animatedRotation, grip);
            Matrix result =
    ...
    public static class AvatarNetworkBridge
    {
        ...
        /// </summary>
        /// <summary>
        /// Give an already-constructed player its imported avatar.
        ///
        /// The patched-executable builds replace the stock model as it is
        /// created, from inside Player..ctor. A mod loader cannot get in that
        /// early, so this runs just after the constructor instead and swaps the
        /// model the game just installed. The observable result is the same,
        /// because nothing has drawn yet, and it goes through the same property
        /// the game itself assigns - which detaches and re-parents correctly.
        ///
        /// Doing nothing when there is no avatar to apply is the normal case:
        /// a remote player's avatar usually arrives seconds later, and
        /// SetRemoteAsset installs it then.
        /// </summary>
        internal static void AdoptPlayer(
            NetworkGamer gamer,
            Avatar avatar,
            Model fallbackModel)
        {
            ...
            NotePlayer(gamer, avatar, fallbackModel);

            string assetPath = GetAssetPath(gamer);
            if (string.IsNullOrEmpty(assetPath) || !File.Exists(assetPath))
            {
                return;
            }
            try
            {
                var replacement = new ImportedAvatarModelEntity(
                    fallbackModel, avatar, assetPath);
                var previous = avatar.ProxyModelEntity as ImportedAvatarModelEntity;
                avatar.ProxyModelEntity = replacement;
                if (previous != null && previous != replacement)
                {
                    previous.ReleaseGraphicsResources();
                }
                NoteAppliedAsset(gamer, assetPath);
                MoveHeldItemAfterModel(avatar);
            }
            catch (Exception exception)
            {
                ImportedAvatarModelEntity.WriteFailure(exception);
        ...
        /// <summary>
        /// Put the prop part last among the avatar's children.
        ///
        /// Entity.Update walks children in order, and a held item records the
        /// world matrix it will draw with during that walk. The anchor
        /// correction runs in the imported model's own update, so the model has
        /// to come first or the item draws with last frame's anchor.
        /// </summary>
        private static void MoveHeldItemAfterModel(Avatar avatar)
        {
            Entity propPart = avatar.GetAvatarPart(AvatarBone.PropRight);
            if (propPart != null && avatar.Children.Remove(propPart))
            {
                avatar.Children.Add(propPart);
            }
        }

        /// <summary>Where this install keeps the imported avatar.</summary>
        internal static string LocalAvatarPath
        {
